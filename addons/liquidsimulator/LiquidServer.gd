...
@export var other_liquidservers: Array = [];
## The liquid scene to be used as a [Liquid] cell.
...
func _ready():
	for child in get_tree().get_nodes_in_group("LiquidServer"):
		if child is LiquidServer and child != self:
			other_liquidservers.append(child);
...
func _process(delta):
	...
	if _time_passed >= refresh_rate:
		...
		check_for_collision_with_other_liquid_server()
		var _updated = _update_simulation();
		refresh_all();
		updated.emit();
		# NOTE TODO: BRAD YOU CHANGED THIS:
		if !_updated:
			stop();
		_time_passed = 0;
		#print(_total_amount);
...
func check_for_collision_with_other_liquid_server():
	for ls in other_liquidservers:
		for cell in _cells:
			if ls.get_liquid(cell.get_x(), cell.get_y()) > 0:
				print("Liquid hit liquid at ", cell.get_x(), cell.get_y())
				# cell.emit_signal("liquid_collision", ls, cell.get_x(), cell.get_y())
				cell.mixed.emit(self, ls, cell.get_x(), cell.get_y())
